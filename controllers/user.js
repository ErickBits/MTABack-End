...
/* Metodo para loguear a un guia */

function saveGuia(req, res){
	var params = req.body;
	var user = new User();
	...
	if (params.name && params.surname && params.nick && params.email && params.password){
		...
		User.find({ $or: [{email: user.email.toLowerCase()},{nick: {$regex : new RegExp(user.nick, "i")}}]}).exec((err, users) => {
			...
			if(users && users.length >= 1){
				users.forEach((bad_user) =>{
					if ((bad_user.nick.toLowerCase() == user.nick.toLowerCase())&&(bad_user.email.toLowerCase() == user.email.toLowerCase())){
						return res.status(200).send({message: 'Ya hay un guia con nick '+user.nick+' y email '+user.email+' registrado!!!'});
					}
					if (bad_user.nick.toLowerCase() == user.nick.toLowerCase()){
						return res.status(200).send({message: 'Ya hay un guia con nick '+user.nick+' registrado!!!'});
					}
					if (bad_user.email.toLowerCase() == user.email.toLowerCase()){
						return res.status(200).send({message: 'Ya hay un guia con email '+user.email+' registrado!!!'});
					}
				});
			} else {
				// Guardamos la contraseña cifrada
